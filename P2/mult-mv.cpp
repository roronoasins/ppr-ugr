...
int main(int argc, char * argv[]) {
  ...
  long **A, // Matriz a multiplicar
          ...
          *misFilas, // La fila que almacena localmente un proceso
  ...
  MPI_Scatter(A[0], // Matriz que vamos a compartir
          n_datos, // Numero de columnas a compartir
  ...
  long *subFinal = new long[n/p];
  for(unsigned int j=0; j < n/p; j++)
  {
    subFinal[j] = 0;
    for (unsigned int i = 0; i < n; i++)
    {
      subFinal[j] += misFilas[i+n*j] * x[i];
      //cout << misFilas[i+n*j] << " * " << x[i] << " = " << misFilas[i+n*j] * x[i]  << " desde " << rank << endl;
      //cout << "acumulado: " << subFinal[j] << " para i: " <<i << "j: " << j << endl;
    }
  }
  ...
  MPI_Gather(subFinal, // Dato que envia cada proceso
          n/p, // Numero de elementos que se envian
          MPI_LONG, // Tipo del dato que se envia
          y, // Vector en el que se recolectan los datos
          n/p, // Numero de datos que se esperan recibir por cada proceso
