...
extern int id;	 // Identificador del proceso dentro de cada comunicador (coincide en ambos)
...
int estado_p;	// Estado del proceso {ACTIVO, PASIVO}
...
void Equilibrar_Carga(tPila &pila, bool &fin, tNodo &solucion)
{
  int proceso_solicitante, count, flag;
  MPI_Status estado;
  ...
  if(pila.vacia())
  {
    MPI_Send(&id, 1, MPI_INT, siguiente, PETICION, MPI_COMM_WORLD); // enviar peticion al proceso id+1%p
    while(pila.vacia() && !fin)
    {
      MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &estado); // esperar mensaje de otro proceso
      switch (estado.MPI_TAG) {
        case PETICION:  // peticion de trabajo
          MPI_Recv(&proceso_solicitante, 1, MPI_INT, anterior, PETICION, MPI_COMM_WORLD, &estado);
          MPI_Send(&proceso_solicitante, 1, MPI_INT, siguiente, PETICION, MPI_COMM_WORLD);
          if(proceso_solicitante == id)
          {
            estado_p = PASIVO;
            if(token_presente) // pasa el testigo
            {
        ...
        case NODOS: // recepcion del trabajo donado
          ...
          MPI_Recv(pila.nodos, count, MPI_INT, estado.MPI_SOURCE, NODOS, MPI_COMM_WORLD, &estado); // recibir los nodos del proceso donante en la pila
          pila.tope = count;
          estado_p = ACTIVO;
          break;
        case TOKEN: // mecanismo de deteccion de fin
          MPI_Recv(&color_token, 1, MPI_INT, siguiente, TOKEN, MPI_COMM_WORLD, &estado);
          token_presente = true;
          if(estado_p == PASIVO) // si el proceso no tiene trabajo
          {
            if(id == 0 && color == BLANCO && color_token == BLANCO) // si el token vuelve al proceso 0 y permanece como blanco, se ha llegado al fin
            {
              fin = true;
              MPI_Send(solucion.datos, 2*NCIUDADES, MPI_INT, siguiente, FIN, MPI_COMM_WORLD); // enviamos fin al siguiente proceso
              MPI_Recv(nueva_solucion.datos, 2*NCIUDADES, MPI_INT, anterior, FIN, MPI_COMM_WORLD, &estado); // esperamos a que hayan recibido el mensaje
        ...
        case FIN: // todos los procesos se quedan sin trabajo
          MPI_Recv(nueva_solucion.datos, 2*NCIUDADES, MPI_INT, anterior, FIN, MPI_COMM_WORLD, &estado);
          fin = true;
          if(nueva_solucion.ci() < solucion.ci()) CopiaNodo(&nueva_solucion, &solucion);
  ...
  {
    MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &flag, &estado); // sondear si hay mensajes pendientes de otros procesos
    while(flag==true) // mientras hay mensajes
    {
      switch (estado.MPI_TAG) {
        case PETICION:
          MPI_Recv(&proceso_solicitante, 1, MPI_INT, anterior, PETICION, MPI_COMM_WORLD, &estado);
          if(pila.tamanio() >= 2) // si hay suficientes nodos en la pila para ceder
          {
            tPila pila_equilibrada;
            pila.divide(pila_equilibrada);
            MPI_Send(pila_equilibrada.nodos, pila_equilibrada.tope, MPI_INT, proceso_solicitante, NODOS, MPI_COMM_WORLD);
            color = (id < proceso_solicitante) ? NEGRO : BLANCO; // el nodo vuelve a tener trabajo
          }else
            MPI_Send(&proceso_solicitante, 1, MPI_INT, siguiente, PETICION, MPI_COMM_WORLD); // reenviar peticion al siguiente
          break;
        case TOKEN:
          MPI_Recv(&color_token, 1, MPI_INT, estado.MPI_SOURCE, TOKEN, MPI_COMM_WORLD, &estado);
          token_presente = true;
          break;
      }
      MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &flag, &estado); // sondear si hay mensajes pendientes de otros procesos
