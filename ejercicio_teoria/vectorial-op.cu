...
// ************************************************
__global__ void vectorial_operation(float * d_phi,
                           float * d_phi_new,
                           int n)
  {
    ...
    if (i<n+2)
    {
      d_phi_new[i-2]= (d_phi[i-2]*d_phi[i-2]+2.0*(d_phi[i-1]*d_phi[i-1]) + d_phi[i]*d_phi[i]
                   -3.0*(d_phi[i+1]*d_phi[i+1]) + 5.0*(d_phi[i+2]*d_phi[i+2]))/24.0;
    }
  }
...
//******************************
  ...
  vectorial_operation_shared_mem<<<blocksPerGrid, BLOCKSIZE>>> (d_A, d_B, n);
